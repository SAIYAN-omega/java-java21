...
public class Lambda_01_Test {
    ...
    public void test_filter_by_firstname() throws Exception {
        ...
        PersonPredicate personPredicate = p -> p.getFirstname().equals("first_10");
        List<Person> result = filter(personList, personPredicate);
    ...
    public void test_filter_by_password() throws Exception {
        ...
        PersonPredicate personPredicate = p -> {
            String MpHasher = DigestUtils.sha512Hex(p.getPassword());
            if(p.getAge() > 49 && MpHasher.equals(passwordSha512Hex))
            {
                return true;
            }
            return false;
